...
class Engine:
    def __init__(self, models_dir: Path):
        ...
        try:
            self.translator = Translator(models_dir)
        except Exception:
...
async def main():
    ...
    # 测试用：把 no_audio / no_speech 的等待时长整体缩短
    ...
    live: dict[str, Any] = {
        ...
        "started_at": 0.0,
        # 会话代：start/switch/stop 各自 +1。重活线程里在途的旧识别结果
        # 到达时对不上代就丢弃，观众的新会话不会漏进旧回调。
        "gen": 0,
        "llm_direct": False,
    }
    ...
    async def ensure_engine():
        nonlocal engine
        if engine is None:
            # 兜底：预加载失败/未完成时首块 PCM 触发同步加载
            loop = asyncio.get_running_loop()
            engine = await loop.run_in_executor(None, lambda: Engine(args.models_dir))
        engine.llm_direct = bool(live.get("llm_direct", False))
        return engine

    async def handle(ws):
        pcm_queue: asyncio.Queue[tuple[int, np.ndarray]] = asyncio.Queue()

        async def send_event(kind, orig, trans, gen):
            # 双闸：换代（start/switch/stop 各自 +1）之后的迟到事件不发；
            # stop 之后到下一次 start 之前的在途事件也不发（观众已经停听了）。
            if not live["started"] or live["gen"] != gen:
                return
            _debug_log(kind, orig, trans)
            await send_obj(ws, {"type": kind, "orig": orig, "trans": trans})

        def threadsafe_cb(kind, gen):
            loop = asyncio.get_running_loop()

            def wrapper(orig, trans):
                if not live["started"] or live["gen"] != gen:
                    return
                # fire-and-forget：同一 loop 的就绪队列保证事件顺序；
                # 在线程里同步等 future 会和 worker 的 await 互相卡死
                asyncio.run_coroutine_threadsafe(send_event(kind, orig, trans, gen), loop)

            return wrapper

        async def heavy_worker():
            """单飞重活：把排队的 PCM 帧合并成一次处理，识别/翻译不阻塞 WS 读；
            帧与会话代一起入队，换代后的残余帧直接丢。"""
            loop = asyncio.get_running_loop()
            while True:
                try:
                    gen, first = await pcm_queue.get()
                    frames = [first]
                    while True:
                        try:
                            item_gen, item = pcm_queue.get_nowait()
                    ...
                    if live["gen"] != gen:
                        continue
                    eng = await ensure_engine()
                    if eng is None:
                        continue
                    pcm = np.concatenate(frames)
                    draft_cb = threadsafe_cb("draft", gen)
                    final_cb = threadsafe_cb("final", gen)
                    eng.busy = True
                    try:
                        await loop.run_in_executor(
                            None, lambda: eng.process(pcm, seg, tnow(), draft_cb, final_cb)
                        )
                    finally:
                        eng.busy = False
                    if seg.last_voice_at is not None:
                        live["last_voice_at"] = seg.last_voice_at
                except asyncio.CancelledError:
                    raise
                except Exception:
                    # 工作协程挂掉等于引擎静默死亡，必须把异常亮出来再继续吃帧
                    traceback.print_exc()

        worker = asyncio.create_task(heavy_worker())
        try:
            async for raw in ws:
                if isinstance(raw, bytes):
                    if not live["started"]:
                        continue
                    if live["got_pcm_at"] is None:
                        live["got_pcm_at"] = tnow()
                    pcm_queue.put_nowait((live["gen"], np.frombuffer(raw, dtype="<f4")))
                    continue
                try:
                    msg = json.loads(raw)
                ...
                if mtype == "start":
                    live["started"] = True
                    live["started_at"] = tnow()
                    live["gen"] += 1
                    live["llm_direct"] = msg.get("translate") == "llm"
                    seg.reset()
                    if engine:
                        engine.reset_session()
                        engine.llm_direct = bool(live["llm_direct"])
                elif mtype == "switch":
                    live["gen"] += 1
                    seg.reset()
                    if engine:
                    ...
                    if engine:
                        await asyncio.to_thread(engine.wait_llm, 3.0)
                    live["started"] = False
                    live["got_pcm_at"] = None
                    live["gen"] += 1
                    if engine:
                        engine.reset_session()
                    seg.reset()
    ...
    async def watchdog(ws):
        """notice 的节流出口：抓不到 / 没人声 / 不是英日韩。"""
        while True:
            await asyncio.sleep(1.0)
            if not live["started"]:
                continue
            now = tnow()
            if live["got_pcm_at"] is None and now - live["started_at"] >= (NO_AUDIO_MS * args.notice_scale) / 1000:
                await send_obj(ws, {"type": "notice", "kind": "no_audio"})
                live["started"] = False  # 与壳的 failed 出口对齐：停
                continue
            if live["got_pcm_at"] is not None:
                silent_for = (live["last_voice_at"] or live["started_at"])
                no_voice = now - silent_for >= (NO_SPEECH_MS * args.notice_scale) / 1000
                if no_voice and now - live["last_no_speech_at"] >= (NO_SPEECH_MS * args.notice_scale) / 1000:
                    live["last_no_speech_at"] = now
                    await send_obj(ws, {"type": "notice", "kind": "no_speech"})
                if engine and engine.empty_hits >= NOT_LANG_EMPTY_HITS and now - engine.last_not_lang_at >= NOT_LANG_COOLDOWN_MS / 1000:
                    engine.last_not_lang_at = now
                    engine.empty_hits = 0
                    await send_obj(ws, {"type": "notice", "kind": "not_lang"})

    async with websockets.serve(
        lambda ws: asyncio.gather(handle(ws), watchdog(ws)),
        "127.0.0.1",
        args.port,
        # 本机回环 + CPU 满载场景下 keepalive 反而会在重活窗口里误杀连接；
        # 存活由壳侧自己管理，这里不发 ping。
        ping_interval=None,
    ) as server:
        ...
        async def preload():
            # 起进程就加载模型：开听时开口→草稿不吃首载延迟
            nonlocal engine
            loop = asyncio.get_running_loop()
            engine = await loop.run_in_executor(None, lambda: Engine(args.models_dir))
            print("LOADED", flush=True)
            # 翻译预热放后台守护线程，句间给重活让位（见 warmup_translation）
            threading.Thread(target=engine.warmup_translation, daemon=True).start()
